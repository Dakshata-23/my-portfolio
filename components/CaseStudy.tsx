...
const CaseStudy: React.FC<CaseStudyProps> = ({ project, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-textPrimary py-12 relative z-10">
      <main className="max-w-5xl mx-auto px-6 lg:px-8">
        {/* Navigation */}
        <nav className="mb-16">
          <button 
            onClick={onBack}
            className="flex items-center gap-2 text-textSecondary hover:text-textPrimary font-medium transition-colors text-sm group"
          >
        ...
        <header className="mb-16">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {project.tags.map(tag => (
              <span key={tag} className="text-xs text-textSecondary bg-surface px-2 py-1 rounded-md font-medium border border-accent">
                #{tag.toLowerCase()}
              </span>
            ))}
          </div>
        ...
        <div className="h-px bg-accent my-16"></div>
        ...
        <article className="space-y-16">
          ...
          {project.caseStudy?.results && (
            <section>
              ...
              <p className="text-base font-medium text-textPrimary leading-relaxed italic border-l-2 border-accent pl-4 py-1">
                "{project.caseStudy.results}"
              </p>
            </section>
          )}
        ...
        {/* Footer Actions */}
        <div className="mt-24 pt-8 border-t border-accent flex flex-col sm:flex-row gap-4 items-center justify-between pb-12">
          {project.link ? (
            <a 
              href={project.link} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="px-6 py-3 bg-textPrimary text-background rounded-full text-sm font-medium hover:scale-105 transition-transform w-full sm:w-auto text-center shadow-md"
            >
              View Live Project
            </a>
          ) : (
          ...
          <button 
            onClick={onBack} 
            className="text-textSecondary text-sm font-medium hover:text-textPrimary transition-colors"
          >
            Close Case Study
          </button>
        </div>
